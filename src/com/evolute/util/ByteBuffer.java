...
public class ByteBuffer {
	...
	public void appendBytes(ByteBuffer bytes, int count) throws Exception {
		if (count > 0) {
			if (bytes == null) {
	...
	public String removeString(int size, String encoding) throws Exception {
		int len = length();
		if (len < size) {
			throw new Exception("NotEnoughDataInByteBufferException");
		}
	...
	public ByteBuffer removeBuffer(int count) throws Exception {
		return removeBytes(count);
	}

	public ByteBuffer removeBytes(int count) throws Exception {
	...
	public void removeBytes0(int count) throws Exception {
		int len = length();
		int lefts = len - count;
		if (lefts > 0) {
	...
	public ByteBuffer readBytes(int count) throws Exception {
		int len = length();
